...
void describe('getVersionOverride', () => {
  void it('should return the overridden version when a string override exists', () => {
    const overrides: Overrides = { 'package-b': '1.0.0' };
    assert.equal(getVersionOverride(overrides, 'package-b'), '1.0.0');
  });

  void it('should return undefined when no override exists for the package', () => {
    const overrides: Overrides = { 'package-b': '1.0.0' };
    assert.equal(getVersionOverride(overrides, 'package-a'), undefined);
  });

  void it('should return undefined when the override is an object (nested), not a string', () => {
    const overrides: Overrides = { 'package-a': { 'package-b': '1.0.0' } };
    assert.equal(getVersionOverride(overrides, 'package-a'), undefined);
  });

  void it('should return undefined for an empty overrides object', () => {
    assert.equal(getVersionOverride({}, 'package-a'), undefined);
  });
});

void describe('getChildOverrides', () => {
  void it('should return global string overrides when child has no nested overrides', () => {
  ...
  void it('should merge global string overrides with nested overrides for a child', () => {
    const rootOverrides: Overrides = {
      'package-c': '2.0.0',
      'package-a': { 'package-b': '1.0.0' },
    };
  ...
  void it('should return only nested overrides merged with global overrides when entering nested package', () => {
    const rootOverrides: Overrides = {
      'package-a': {
  ...
  void it('should return empty object when both overrides are empty', () => {
    const result = getChildOverrides({}, {}, 'package-a');
    assert.deepEqual(result, {});
  });

  void it('should not include nested object overrides in global overrides propagation', () => {
    const rootOverrides: Overrides = {
      'package-a': { 'package-b': '1.0.0' },
    };
...
void describe('isOverrides', () => {
  void it('should return true for a flat string-valued overrides object', () => {
    assert.equal(isOverrides({ foo: '1.0.0' }), true);
  });

  void it('should return true for a nested overrides object', () => {
    assert.equal(isOverrides({ 'package-a': { 'package-b': '1.0.0' } }), true);
  });

  void it('should return true for an empty object', () => {
  ...
  void it('should return false for a non-object value', () => {
    assert.equal(isOverrides('1.0.0'), false);
    assert.equal(isOverrides(42), false);
  });

  void it('should return false when a value is neither a string nor an object', () => {
